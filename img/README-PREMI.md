# 📸 IMMAGINI PREMI LOTTERIA

## 🎯 File Richiesti

### 🥇 PRIMO PREMIO
**File:** `1premio1.jpg`  
**Prodotto:** MIKELI GENOA - Giacca imbottita da uomo  
**Modello:** Blue dk-red blaze  
**Prezzo:** €149,00  
**Descrizione:** Giacca imbottita da uomo blu scuro con logo della squadra

### 🥈 SECONDO PREMIO  
**File:** `2premio1.jpg` (principale), `2premio2.jpg`, `2premio3.jpg` (dettagli)
**Prodotto:** MEQUO GENOA - Giacca in pile da uomo
**Modello:** Blue dk-red blaze  
**Prezzo:** €79,00
**Descrizione:** Giacca in pile blu da uomo con cappuccio e vestibilità slim

### 🥉 TERZO PREMIO
**File:** `3premio1.jpg` (principale), `3premio2.jpg` (dettagli)
**Prodotto:** 20.3H GENOA - Pallone da calcio da allenamento
**Modello:** White-red-blue dk-yellow
**Prezzo:** €30,00
**Descrizione:** Pallone da calcio da allenamento unisex grigio a 14 pannelli

## 📐 Specifiche Tecniche
- **Formato:** JPG o PNG
- **Risoluzione:** Minimo 800x600px  
- **Proporzioni:** Preferibile 4:3 o 16:10
- **Dimensione file:** Max 2MB per ottimizzazione web

## 🎨 Suggerimenti per le Foto
- Sfondo neutro o bianco
- Illuminazione uniforme
- Prodotto ben visibile e centrato  
- Logo Genoa CFC ben leggibile
- Evitare ombre eccessive

## 🔗 Implementazione
Le immagini vengono caricate automaticamente nella galleria premium con:
- Effetti hover 3D
- Bordi dorati animati  
- Zoom al passaggio del mouse
- Layout responsive per mobile

**Totale valore premi: €258,00** 🎄✨